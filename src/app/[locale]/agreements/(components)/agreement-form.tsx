...
import { agreementSchema, AgreementSchema } from '@/lib/schema/agreement.schema'
...
import { createAgreement, updateAgreement } from '../agreement.action'
...
export const AgreementForm = ({ mode = formMode.create, data }: Props) => {
  ...
  const methods = useForm<AgreementSchema>({
  ...
  const onSubmit = async (values: AgreementSchema) => {
    try {
      toast.info(t('creating'))
      if (mode === formMode.edit && data?.id) {
        await updateAgreement(data.id, values)
