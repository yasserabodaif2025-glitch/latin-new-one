...
import {
  ...
  QualificationTypeSchema,
} from '@/lib/schema/qualification-type.schema'
...
import { createQualificationType, updateQualificationType } from '../qualification-type.action'
...
export const QualificationTypeForm = ({ mode = formMode.create, data }: Props) => {
  ...
  const methods = useForm<QualificationTypeSchema>({
    resolver: zodResolver(qualificationTypeSchema),
    defaultValues,
  ...
  const onSubmit = async (values: QualificationTypeSchema) => {
    try {
      toast.info(t('creating'))
      if (mode === formMode.edit && data?.id) {
        await updateQualificationType(data?.id ?? 0, values)
        toast.success(t('editSuccess'))
      } else {
        await createQualificationType(values)
        toast.success(t('addSuccess'))
      }

      router.push(`/${routes.qualificationType}`)
    } catch (error) {
      console.error(error)
    }
  }
        ...
        <div className="flex items-center justify-between">
          ...
          <div>
            <div className="mt-4 flex items-center justify-start gap-3 [&_button]:px-10">
              ...
              <Link href={`/${routes.qualificationType}`}>
