              ...
              <FormItem>
                ...
                <Select onValueChange={field.onChange} value={String(field?.value)}>
                  ...
                  <SelectContent>
                    ...
                    {/* render any students with missing id using a stable index-based fallback to avoid undefined keys */}
                    {students
                      .map((s, i) => ({ s, i }))
                      .filter(({ s }) => s?.id === undefined || s?.id === null)
                      .map(({ s, i }) => (
                        <SelectItem key={`student-fallback-${i}`} value={`student-fallback-${i}`}>
                          {s?.name ?? `طالب ${i + 1}`}
                        </SelectItem>
                      ))}
                    <SelectSeparator />
                    ...
                    <Select onValueChange={field.onChange} defaultValue={String(field.value)}>
                      ...
                      <SelectContent>
                        ...
                        {enrollments
                          .map((e, i) => ({ e, i }))
                          .filter(
                            ({ e }) => e?.enrollmentId === undefined || e?.enrollmentId === null
                          )
                          .map(({ e, i }) => (
                            <SelectItem
                              key={`enrollment-fallback-${i}`}
                              value={`enrollment-fallback-${i}`}
                            >
                              {e?.groupName ?? `قيد ${i + 1}`}
                            </SelectItem>
                          ))}
